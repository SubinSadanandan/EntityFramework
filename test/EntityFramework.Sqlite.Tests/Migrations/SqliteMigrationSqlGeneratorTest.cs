...
namespace Microsoft.Data.Entity.Sqlite.Migrations
{
    public class SqliteMigrationSqlGeneratorTest : MigrationSqlGeneratorTestBase
    {
        ...
        [Fact]
        public void AddColumnOperation_throws_with_current_date()
        {
            var ex = Assert.Throws<NotSupportedException>(() => base.AddColumnOperation_with_defaultValueSql());

            Assert.Equal(Strings.AddColumnWithDefaultExprNotSupported("People", "CURRENT_TIMESTAMP"), ex.Message);
        }
        ...
        public void CreateSchemaOperation_not_supported()
        {
            ...
            Assert.Equal(Strings.MultipleDatabasesNotSupported, ex.Message);
        }
